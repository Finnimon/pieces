...
public class SCity {
    ...
    public static Scene createCityScene(Stage stage) {
        ...
        Scene scene = new Scene(grid, Constants.DEFAULT_WIDTH * Constants.MAP_DIMENSION, Constants.DEFAULT_WIDTH * Constants.MAP_DIMENSION);
                ...
                switch (map[i][j]) {
                    ...
                    case 1 -> {
                        ...
                        pane.setOnMouseEntered(e -> {
                            if ((Math.abs(finalJ - playerY) == 1 && Math.abs(finalI - playerX) == 0) ||
                                    (Math.abs(finalJ - playerY) == 0 && Math.abs(finalI - playerX) == 1)) {
                                grid.setCursor(Cursor.HAND);
                            }
                        });
                        pane.setOnMouseExited(e -> grid.setCursor(Cursor.DEFAULT));
                                ...
                                try {
                                    if (playerX < Constants.MAP_DIMENSION - 1 && map[playerX + 1][playerY] == 1)
                                        getGridCell(grid, playerX + 1, playerY).setBorder(
                                                new Border(new BorderStroke(Color.LIGHTGREEN, BorderStrokeStyle.SOLID,
    ...
    private static Pane getGridCell(GridPane pane, int x, int y) {
        for (Node node : pane.getChildren()) {
            if (GridPane.getRowIndex(node) != null && GridPane.getRowIndex(node) == x &&
